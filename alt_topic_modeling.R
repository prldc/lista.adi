...
stop <- c("poder", "cf", "moreira", "alves", "tribunal", "infraestrutura", "dje", "l",
          ...
          "to", "ma", "pi", "pe", "al", "se", "sp", "pr", "go", "la", "ex", "ª", "ã", "nã", "â", "nâ", "ãµes",
          "nãºmero", "prã")

plan2020 <- read_csv("~/R Projects/stf_text_mining/planilha2020_processada-dupl_clusters.csv")
...
lda <- LDA(dfm, k = 49, control = list(seed = pi))  # Creates a LDA model with 6 topics.
...
result <- FindTopicsNumber(
  dfm,
  topics = c(9, 16, 25, 36, 49),
  metrics = c("Griffiths2004", "CaoJuan2009", "Arun2010", "Deveaud2014"),
  method = "Gibbs",
  control = list(seed = pi),
  mc.cores = 6L,
  verbose = TRUE
)
