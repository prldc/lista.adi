...
plan2020 <- read_csv("planilha2020_processada*.csv")  # Reads the table.
...
rect.hclust(hc, 16, border = "turquoise")  # Draws clusters.
rect.hclust(hc, 32, border = "green")  # Draws clusters.
rect.hclust(hc, 64, border = "firebrick")  # Draws clusters.
rect.hclust(hc, 128, border = "orange")  # Draws clusters.
...
nome_ementa <- plan2020 %>% select(nome, data_julgamento, ementa, tipo_julgamento, cluster_128, dispositivo, resultado)
...
plan2020<- plan2020 %>% mutate(unam=case_when(placar == "unânime" ~ T,
                                   T ~ F))
...
(taxa_unanimidade_trad <- sum(tradicional$unam)/nrow(tradicional))

# CHISQ

tbl <- table(plan2020$tipo_julgamento, plan2020$placar)
...
u <- matrix(c(137,160,94,107,17, 43),nrow=3,ncol=2) 
df <- data.frame (tipo  = c("tradicional", "lista", "virtual"),
                  unânime = c(137, 160, 94),
                  maioria = c(107, 17, 42))
)

c<- plan2020 %$%
  ctable(tipo_julgamento, placar,
...
z <- plan2020 %>% group_by(tipo_julgamento, unam) %>% count()

p <- ggplot(data = z,
            mapping = aes(x = tipo_julgamento, fill = unam, y=n)) + geom_col(position="dodge")

p + labs(fill = "Unanimidade", title = "Número de julgamentos unânimes", x="Tipo de julgamento")

# plan2020 %>%
#   group_by(tipo_julgamento) %>% 
#   summarise("TRUE" = mean(unam),
#             "FALSE" = mean(!unam))
  
ggbarstats(data = plan2020, x = placar, y = tipo_julgamento) + labs(caption = NULL, title="Teste qui-quadrado de Pearson",
                                                                    fill="Placar", x="Tipo de julgamento")
chisq.test(table(plan2020$tipo_julgamento, plan2020$placar))
